<?php
	...
	try{
		...
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$request = "CREATE TABLE IF NOT EXISTS `users` (`id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,`username` VARCHAR(8) NOT NULL,`password` VARCHAR(255) NOT NULL,`email` VARCHAR(50) NOT NULL, `confirmed` VARCHAR(1) NOT NULL DEFAULT 'N')";
		$pdo->prepare($request)->execute();
	}
	catch(PDOException $Exception){
